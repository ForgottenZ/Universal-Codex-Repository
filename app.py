...
class User(db.Model):
    ...
    email_addr = db.Column(db.String(256), default="")
    ...
    email_subject = db.Column(db.String(256), default="教学周提醒")
...
def _resolve_email_profile(user: User) -> dict:
    ...
    to_addr = (cfg.get("to_addr") or user.email_addr or username).strip()
    subject = (cfg.get("subject") or user.email_subject or "教学周提醒").strip()
...
def send_graph_email(user: User, text: str):
    ...
    recipient = profile.get("to_addr")
    subject = profile.get("subject") or "教学周提醒"
    if not sender:
        log(f"Graph邮件跳过 user={user.username}（缺少 from_addr/username）", "WARN")
        return
    if not recipient:
        log(f"Graph邮件跳过 user={user.username}（缺少收件人邮箱）", "WARN")
        return
...
def setup_admin():
    ...
    if request.method == "POST":
        ...
        if not username or not nickname or not email_addr or not password:
            flash("请填写完整管理员信息")
            return render_template("setup_admin.html")
        admin = User(username=username, nickname=nickname, email_addr=email_addr, password=password, is_admin=True)
...
def save_config():
    ...
    user.email_addr = request.form.get("email_addr", user.email_addr or "")
    user.email_subject = request.form.get("email_subject", user.email_subject or "教学周提醒")
...
def create_user():
    ...
    if not username or not nickname or not password or not email_addr:
        flash("用户名/称呼/注册邮箱/密码不能为空")
        return redirect(url_for("index"))
    if User.query.filter_by(username=username).first():
        flash("用户名已存在")
        return redirect(url_for("index"))
    db.session.add(User(username=username, nickname=nickname, email_addr=email_addr, password=password, is_admin=is_admin))
    db.session.commit()
    flash("用户创建成功")
    return redirect(url_for("index"))
...
def ensure_schema_updates():
    # 仅对 sqlite 做轻量补字段；MySQL 建议走正式迁移工具
    if db.engine.dialect.name != "sqlite":
        return
    with db.engine.begin() as conn:
        cols = [row[1] for row in conn.execute(text("PRAGMA table_info(user)")).fetchall()]
        if "email_addr" not in cols:
            conn.execute(text("ALTER TABLE user ADD COLUMN email_addr VARCHAR(256) DEFAULT ''"))
        if "email_subject" not in cols:
            conn.execute(text("ALTER TABLE user ADD COLUMN email_subject VARCHAR(256) DEFAULT '教学周提醒'"))
